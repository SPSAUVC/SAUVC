...
while True:      
    ...
    if (now > 0 and desire_angle_addition_Bool == False):
        g = open("rotation.txt", "r") #Reads 'rotation' state file.
...
#                 print("Gate @ right")
    ...
    #If error is less than 5 degrees
    if (0< error[0] <5):
        ...
        if (f.mode == 'r'):
            #Read 'distance' file
            distance_txtfile = f.read()
            distance = float(distance_txtfile)
            desired_time = distance/0.5454 #In seconds
        ...
        if (time > desired_time):
            #stop()
            import CompetitionCode_Path1_part3.py
            break
